...
app.get("/api/search/:productName", async (req, res) => {
  try {
    ...
    if (allReviews.length === 0) {
      ...
      console.log(
        `No actual reviews found for ${productName}, using ${fallbackReviews.length} fallback reviews`
      );
    ...
    console.log(
      `Found a total of ${allReviews.length} review(s) for ${productName}, proceeding with analysis`
    );
...
app.post("/api/url", async (req, res) => {
  try {
    ...
    // Log the number of reviews before analysis
    console.log(
      `Found a total of ${reviews.length} review(s) for URL: ${productUrl}, proceeding with analysis`
    );
    ...
    // Send response
    console.log(
      `Sending response with ${reviews.length} review(s) and analysis`
    );
    res.json(responseData);
  } catch (error) {
...
async function scrapeAmazonReviewsFromUrl(url) {
  try {
    ...
    let reviewsUrl = $(
      'a#customerReviews, a#cr-pagination-link-top, a[data-hook="see-all-reviews-link-foot"]'
    ).attr("href");
    ...
    // Get reviews page
    const { html: reviewsHtml, source: reviewsSource } = await fetchHtml(
      reviewsUrl
    );
      ...
      try {
        ...
        if (nextPageLink) {
          const nextPageUrl = nextPageLink.startsWith("http")
            ? nextPageLink
            : `https://www.amazon.com${nextPageLink}`;
        ...
        console.error(
          "Error fetching additional Amazon reviews:",
...
async function scrapeFlipkartReviewsFromUrl(url) {
  try {
    ...
    reviewElements.each((i, elem) => {
      ...
      const reviewText = reviews$(elem)
        .find("div.t-ZTKy div div")
        .text()
        .trim();
      let rating = "0";
    ...
    if (reviews.length < 20) {
      try {
        ...
        if (nextPageLink) {
          const nextPageUrl = nextPageLink.startsWith("http")
            ? nextPageLink
            : `https://www.flipkart.com${nextPageLink}`;
        ...
        console.error(
          "Error fetching additional Flipkart reviews:",
          paginationError.message
        );
      }
    }
    ...
    console.log(
      `Found ${reviews.length} Flipkart reviews for "${productName}"`
    );
    return reviews;
  } catch (error) {
  ...
  try {
    ...
    if (reviews.length < 20) {
      try {
        ...
        const moreReviewsLink = $(
          'a:contains("View more reviews"), a:contains("See all reviews")'
        ).attr("href");
        if (moreReviewsLink) {
          const moreReviewsUrl = moreReviewsLink.startsWith("http")
            ? moreReviewsLink
            : `https://www.google.com${moreReviewsLink}`;
          ...
          const { html: moreReviewsHtml } = await fetchHtml(moreReviewsUrl, {
        ...
        console.error(
          "Error fetching additional Google reviews:",
          moreReviewsError.message
        );
      }
    }
...
async function scrapeGenericReviewsFromUrl(productUrl) {
  try {
    ...
    if (reviews.length === 0) {
      console.log(
        "No reviews found with specific selectors, trying simple text extraction"
      );
...
async function scrapeAmazonReviews(productName) {
  try {
    ...
    console.log(
      `Got Amazon search results using ${source} method (${html.length} bytes)`
    );
    ...
    for (const selector of standardSelectors) {
      ...
      console.log(
        `Amazon selector "${selector}" found ${links.length} elements`
      );
      ...
      if (links.length > 0) {
        links.each((i, element) => {
          ...
          if (href && !firstProductLink) {
            console.log(
              `Found Amazon product link using selector "${selector}": ${href.substring(
                0,
                50
              )}...`
            );
    ...
    if (!firstProductLink) {
      ...
      $("a").each((i, element) => {
        ...
        if (
          ...
          console.log(
            `Found Amazon product link using ASIN pattern: ${href.substring(
              0,
              50
            )}...`
          );
...
async function scrapeFlipkartReviews(productName) {
  try {
    ...
    console.log(
      `Got Flipkart search results using ${source} method (${html.length} bytes)`
    );
    ...
    for (const selector of selectors) {
      ...
      console.log(
        `Flipkart selector "${selectorStr}" found ${elements.length} elements`
      );
      ...
      elements.each((i, element) => {
        ...
        if (href) {
          ...
          if (!firstProductLink) {
            console.log(
              `Found Flipkart product link using selector "${selectorStr}": ${href.substring(
                0,
                50
              )}...`
            );
    ...
    if (!firstProductLink) {
      ...
      $("a").each((i, element) => {
        ...
        if (href && (href.includes("/p/") || href.includes("pid="))) {
          ...
          if (!firstProductLink) {
            console.log(
              `Found Flipkart product link using generic pattern: ${href.substring(
                0,
                50
              )}...`
            );
...
async function scrapeGoogleReviews(productName) {
  try {
    ...
    console.log(
      `Got Google search results using ${source} method (${html.length} bytes)`
    );
    ...
    for (const selector of selectors) {
      ...
      console.log(
        `Google selector "${selector}" found ${elements.length} elements`
      );
      ...
      elements.each((i, element) => {
        ...
        if (href) {
          ...
          if (!productLink) {
            console.log(
              `Found Google product link using selector "${selector}": ${href.substring(
                0,
                50
              )}...`
            );
    ...
    if (!productLink) {
      ...
      $("a").each((i, element) => {
        ...
        if (
          ...
          if (!productLink) {
            console.log(
              `Found Google product link using generic pattern: ${href.substring(
                0,
                50
              )}...`
            );
    ...
    if (!productLink) {
      console.log(
        "No products found in Google Shopping, trying regular search..."
      );

      // Do a regular Google search for product reviews
      const regularSearchUrl = `https://www.google.com/search?q=${encodeURIComponent(
        productName
      )}+review`;
      ...
      regularSearch$("a").each((i, element) => {
        ...
        if (
          ...
          if (!productLink) {
            console.log(
              `Found review link in regular search: ${href.substring(0, 50)}...`
            );
            productLink = href;
          }
        }
      ...
      try {
        const urlObj = new URL(
          `https://www.google.com${
            productLink.startsWith("/") ? productLink : "/" + productLink
          }`
        );
        const actualUrl = urlObj.searchParams.get("q");
        if (actualUrl) {
          console.log(
            `Extracted actual URL from Google redirect: ${actualUrl.substring(
              0,
              50
            )}...`
          );
    ...
    try {
      ...
      console.log(
        "Google specialized scraper returned no reviews, trying generic approach"
      );
    } catch (error) {
...
async function analyzeReviewsWithAI(reviews, productName) {
  try {
    ...
    const reviewTexts = reviews
      .map(
        (review) =>
          `Platform: ${review.platform || "Unknown"}, Rating: ${
            review.rating || "Not rated"
          }, Review: ${review.content || review.text || "No content"}`
      )
      .join("\n\n");
...
Make sure to return only the JSON object, without any additional text or explanation.`;
        ...
        prompt = `You are a product review analyzer. Analyze these ${
          reviews.length
...
function generateLocalAnalysis(reviews, productName) {
  ...
  let summary;
  if (avgRating >= 4) {
    summary = `Overall, ${productName} has received positive feedback with an average rating of ${avgRating.toFixed(
      1
    )}. Users generally seem satisfied with their purchase.`;
  } else if (avgRating >= 3) {
    summary = `Reviews for ${productName} are mixed, with an average rating of ${avgRating.toFixed(
      1
    )}. Some users are satisfied while others have concerns.`;
  } else {
    summary = `Based on reviews, ${productName} has received mostly negative feedback with an average rating of ${avgRating.toFixed(
      1
  ...
  return {
    ...
    pros: [
      avgRating >= 4
        ...
        : "Some positive aspects mentioned",
...
function getMockReviews(platform, count = 3) {
  const mockData = {
    Amazon: [
      {
        content:
          "Great product, very satisfied with the purchase. The quality is excellent and it exceeded my expectations in every way.",
        rating: 5,
        date: new Date(),
      },
      {
        content:
          "Decent quality but a bit overpriced compared to similar items. Works well enough but I expected better for the price.",
        rating: 3,
        date: new Date(),
      },
      {
        content:
          "The product arrived damaged and customer service was unhelpful. Had to return it and the process was frustrating.",
        rating: 1,
        date: new Date(),
      },
      {
        content:
          "Amazing product and fast shipping! Would definitely recommend to anyone considering this purchase.",
        rating: 5,
        date: new Date(),
      },
      {
        content:
          "Product works exactly as described. No complaints so far after several weeks of use.",
        rating: 4,
        date: new Date(),
      },
      {
        content:
          "Instructions were confusing and setup was difficult. Once I got it working, the performance was okay.",
        rating: 3,
        date: new Date(),
      },
      {
        content:
          "Not worth the money at all. Poor build quality and stopped working after just a few uses.",
        rating: 2,
        date: new Date(),
      },
    ],
    Flipkart: [
      {
        content:
          "Good value for money, fast delivery. The packaging was secure and the product was exactly as shown in pictures.",
        rating: 4,
        date: new Date(),
      },
      {
        content:
          "Product broke after two weeks of use. Very disappointed with the durability.",
        rating: 1,
        date: new Date(),
      },
      {
        content:
          "Excellent product with great features. This is the best version of this product I have used so far.",
        rating: 5,
        date: new Date(),
      },
      {
        content:
          "The product is fine but delivery took longer than expected. Flipkart should improve their shipping times.",
        rating: 3,
        date: new Date(),
      },
      {
        content:
          "Perfect product for my needs. Easy to use and very efficient. Would buy again from this seller.",
        rating: 5,
        date: new Date(),
      },
      {
        content:
          "Not as described in the product details. The color is different and some features are missing.",
        rating: 2,
        date: new Date(),
      },
      {
        content:
          "Average product for the price. There are better alternatives available but this works fine for basic use.",
        rating: 3,
        date: new Date(),
      },
    ],
    Google: [
      {
        content:
          "Excellent customer service and product quality. The company really cares about their customers.",
        rating: 5,
        date: new Date(),
      },
      {
        content:
          "Average product, nothing special but works as advertised. No major issues to report.",
        rating: 3,
        date: new Date(),
      },
      {
        content:
          "Would recommend to friends, good quality for the price. It has been reliable so far in my daily use.",
        rating: 4,
        date: new Date(),
      },
      {
        content:
          "Product design is sleek and modern. Performance is consistently good across all features.",
        rating: 5,
        date: new Date(),
      },
      {
        content:
          "Battery life is disappointing compared to what was advertised. Otherwise functions well.",
        rating: 3,
        date: new Date(),
      },
      {
        content:
          "Poor customer support when I had issues. Was on hold for hours and problem still not resolved.",
        rating: 2,
        date: new Date(),
      },
    ],
  ...
  try {
    ...
    if (browser && (options.preferPuppeteer || options.forcePuppeteer)) {
      try {
        ...
        console.log(
          `Successfully fetched content from ${normalizedUrl} using Puppeteer (${html.length} bytes)`
        );
        return { html, source };
      } catch (puppeteerError) {
        console.error(
          `Puppeteer error for ${normalizedUrl}:`,
    ...
    if (process.env.SCRAPER_API_KEY) {
      try {
        ...
        }&url=${encodeURIComponent(normalizedUrl)}&render=true&retry=3`;
        ...
        if (response.status === 200 && response.data) {
          console.log(
            `Successfully fetched content from ${normalizedUrl} using ScraperAPI`
          );
          source = "scraperapi";
        ...
        console.error(
          `ScraperAPI error for ${normalizedUrl}:`,
    ...
    console.log(
      `Successfully fetched content from ${normalizedUrl} using direct request`
    );
    source = "direct";
    return { html: response.data, source };
  } catch (error) {
...
async function extractReviews(html, platform) {
  ...
  const extractionStrategies = {
    ...
    Flipkart: {
      ...
      rating: ["div._3LWZlK", "div.review-rating", "span.rating"],
      date: ["p._2sc7ZR", "span.review-date", "div.review-date"],
    },
    Google: {
      container: [
        "div.HMeEI",
      ...
      rating: ["span.UzThIf", "div.review-rating", "span.rating"],
      date: ["span.review-date", "div.review-date"],
    },
    Generic: {
      container: [
        "div.review",
  ...
  const strategy =
    extractionStrategies[platform] || extractionStrategies["Generic"];
  ...
  if (reviews.length === 0) {
    console.log(
      `No reviews found with specific selectors for ${platform}, trying simple text extraction`
    );
...
app.get("/api/test-scraper", async (req, res) => {
  try {
    ...
    if (!platform || !query) {
      return res
        .status(400)
        .json({ error: "Platform and query parameters are required" });
    }
    ...
    switch (platform.toLowerCase()) {
      ...
      default:
        return res
          .status(400)
          .json({
            error: 'Invalid platform. Use "amazon", "flipkart", or "google"',
          });
    }
...
(async () => {
  try {
    ...
    if (!browserInitialized) {
      console.warn(
        "Warning: Puppeteer browser initialization failed. The application will continue without browser-based scraping."
      );
    }
    ...
    app.listen(PORT, () => {
      ...
      console.error(" ");
      console.error(
        " This application performs web scraping, which may violate"
      );
      ...
      console.error(
        " the respective platforms or obtaining proper permissions."
      );
      console.error("------------------------------------------------");
      ...
      if (browserInitialized)
        console.log("✓ Puppeteer (browser-based scraping)");
      else console.log("✗ Puppeteer (not available)");

      if (process.env.SCRAPER_API_KEY)
        console.log("✓ ScraperAPI (API-based scraping)");
      else
        console.log(
          "✗ ScraperAPI (not configured - add SCRAPER_API_KEY to .env)"
        );
